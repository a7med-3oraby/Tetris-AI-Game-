...
#         """Calculate the maximum height of the Tetris board."""
#         max_height = 0
#         for col in range(len(board)):
#             height = 0
#             for row in range(len(board[0])):
#                 if board[col][row] != BLANK:
#                     height = len(board[0]) - row
#                     break
#             if height > max_height:
#                 max_height = height
#         return max_height

# def calc_difference_max_height(board, piece):
#         """Calculate the difference in maximum height between the current board and the board with the next piece."""
#         next_board = [row[:] for row in board]  # Create a copy of the board
#         add_to_board(next_board, piece)  # Add the next piece to the board
#         current_max_height = calc_max_height(board)
#         next_max_height = calc_max_height(next_board)
#         return next_max_height - current_max_height

# def calc_min_height(board):
#         """Calculate the minimum height of the Tetris board."""
#         min_height = len(board[0])  # Initialize with maximum possible value
#         for col in range(len(board)):
#             for row in range(len(board[0])):
#                 if board[col][row] != BLANK:
#                     height = len(board[0]) - row
...
# def calc_num_gaps(board):
#         """Calculate the number of gaps in the Tetris board."""
#         num_gaps = 0
#         for col in range(len(board)):
#             for row in range(1, len(board[0])):
#                 if board[col][row] == BLANK and board[col][row - 1] != BLANK:
#                     num_gaps += 1
#         return num_gaps

# def calc_score_gained(board, piece, x, r):
        # """Calculate the score gained by placing a piece with a given position and rotation on the board."""
        # new_board = [row[:] for row in board]  # Create a copy of the board
        # piece['x'] = x
        # piece['rotation'] = r
        # add_to_board(new_board, piece)  # Add the piece to the board
        # num_removed_lines = remove_complete_lines(new_board)  # Remove complete lines
        # score_gained = 40 * num_removed_lines  # Score calculation based on removed lines
        # return score_gained